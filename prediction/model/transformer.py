...
class TrajectoryTrainer:
    ...
    def _compute_loss(
        ...
        """Compute MSE loss on non-padded positions."""
        ...
        return loss.sum() / (expanded_mask.sum() + 1e-9)
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        with tqdm(self.train_loader, desc="Training") as pbar:
            for batch_idx, (masked_input, attention_mask, target) in enumerate(pbar):
    ...
    def validate(self) -> Dict[str, float]:
        ...
        with tqdm(self.val_loader, desc="Validating") as pbar:
            for batch_idx, (src, tgt) in enumerate(pbar):
                ...
                src_padding_mask = src[:, :, 0] <= -900
                tgt_padding_mask = tgt[:, :, 0] <= -900
    ...
    def train(self, num_epochs: int):
        ...
        if self.use_wandb:
            wandb.init(project="trajectory-prediction")
