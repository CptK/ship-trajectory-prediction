...
from prediction.preprocessing.utils import max_bbox_diagonal, pairwise_point_distances
...
def filter_by_travelled_distance(
    ...
    method: Literal["exact", "approximate"] = "approximate",
    ...
    Args:
        ...
        method: Method to use for distance calculation. Either "exact" or "approximate". If "exact", the
            haversine formula is used to calculate the distance between each pair of points. If "approximate",
            the distance is calculated as the maximum diagonal of the bounding box of the trajectory.
    ...
    if dist_col is None:
        ...
        if method == "exact":
            ...
            # Calculate distances using bounding box diagonal
            data["max_dist"] = (
                ddf["geometry_wkt"]
                .map(lambda x: max_bbox_diagonal(wkt.loads(x)), meta=("max_dist", "float64"))
                .compute()
            )
