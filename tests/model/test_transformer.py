...
from prediction.model.transformer import TrajectoryEmbedding, TrajectoryEncoder, TrajectoryDecoder, TrajectoryTransformer
...
class TestTrajectoryEmbedding(TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertEqual(
            self.embedding.input_projection.in_features,
            self.input_dim
        )
        self.assertEqual(
            self.embedding.input_projection.out_features,
            self.d_model
        )
        self.assertEqual(
            self.embedding.norm.normalized_shape[0],
            self.d_model
        )
    ...
    def test_positional_encoding(self):
        ...
        self.embedding.input_projection = nn.Identity()
...
class TestTrajectoryEncoder(TestCase):
    ...
    def test_initialization(self):
        ...
        with self.assertRaises(ValueError):
            TrajectoryEncoder(
                d_model=127, nhead=8, num_layers=3, dim_feedforward=512)
            
        # Test invalid dropout
        with self.assertRaises(ValueError):
            TrajectoryEncoder(
                d_model=128, nhead=8, num_layers=3, dim_feedforward=512, dropout=1.5)
...
class TestTrajectoryDecoder(TestCase):
    ...
    def test_invalid_initialization(self):
        ...
        with self.assertRaises(ValueError):
            TrajectoryDecoder(
                d_model=255,  # Not divisible by nhead
                nhead=8,
                num_layers=3,
                dim_feedforward=1024
            )
...
class TestTrajectoryTransformer(TestCase):
    ...
    def test_forward_with_padding_mask(self):
        ...
        self.assertNotEqual(
            masked_output.mean().item(),
            unmasked_output.mean().item()
        )
