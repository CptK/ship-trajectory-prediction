...
class TestMaskedAISDataset(TestCase):
    ...
    def test_random_masking(self):
        ...
        expected_masks = max(self.dataset.min_masks, 
                           int(seq_len * self.dataset.random_mask_ratio))
    ...
    def test_consecutive_masking(self):
        ...
        # Check number of masked positions
        expected_masks = max(self.dataset.min_masks, 
                           int(seq_len * self.dataset.consecutive_mask_ratio))
        self.assertEqual(mask.sum(), expected_masks)
    ...
    def test_end_masking(self):
        ...
        # Check number of masked positions
        expected_masks = max(self.dataset.min_masks, 
                           int(seq_len * self.dataset.end_mask_ratio))
        self.assertEqual(mask.sum(), expected_masks)
        
        # Verify end positions are masked
        self.assertTrue(
            np.all(mask[-expected_masks:]),
            "End positions should be masked"
        )
        self.assertTrue(
            np.all(~mask[:-expected_masks]),
            "Non-end positions should not be masked"
        )
    ...
    def test_strategy_distribution(self):
        ...
        self.dataset._create_masks = counting_create_masks
        ...
        self.assertAlmostEqual(
            strategy_counts[0] / n_samples, 0.2, delta=0.05,
            msg="Random masking frequency should be ~20%"
        )
        self.assertAlmostEqual(
            strategy_counts[1] / n_samples, 0.3, delta=0.05,
            msg="Consecutive masking frequency should be ~30%"
        )
        self.assertAlmostEqual(
            strategy_counts[2] / n_samples, 0.5, delta=0.05,
            msg="End masking frequency should be ~50%"
        )
        
        # Restore original method
        self.dataset._create_masks = original_create_masks
    ...
    def test_getitem_output(self):
        ...
        self.assertEqual(
            masked_input.shape,
            (self.dataset.max_seq_len, 6)  # 2(lat,lon) + 1(speed) + 2(sin,cos) + 1(time)
        )
        ...
        # Check padding
        n_padded = self.dataset.max_seq_len - 50  # 50 is original sequence length
